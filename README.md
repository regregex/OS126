...
`7876affd61086a3480b118a6ddcf33cf`.
...
Pointing a \*command at `CLIEND` (&E092) passes it to paged ROMs or the
...
(&E09B) sends the command straight to the filing system control vector,
defined at &021E.

`JMIUSR` (&E6B6) sends a \*command to USERV, defined at &0200.  An
...
In a routine handling the new command, `SKIPSP` (&E0AB) may be passed
...
`SKIPSN` (&E0AA) is the same but ignores the current character by
advancing Y over it.
...
- Slogger's Tape to Challenger 3 ROM (T2C3) 1.00 jumps to the hard-coded
  ...
  call to OSBYTE. (Patch &8F15 = `JMP &E7CD`.)
