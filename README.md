...
- 521 bytes cleared in the main section + 1 existing = 522 bytes free
...
`03db06ab844f58810b3c288064ef0564`.
...
Pointing a \*command at `CLIEND` (&E140) passes it to paged ROMs or the
...
(&E149) sends the command straight to the filing system control vector,
defined at &021E.
...
In a routine handling the new command, `SKIPSP` (&E159) may be passed
...
`SKIPSN` (&E158) is the same but ignores the current character by
advancing Y over it.
...
- Slogger's Tape to Challenger 3 ROM (T2C3) 1.00 jumps to the hard-coded
  ...
  call to OSBYTE. (Patch &8F15 = `JMP &E876`.)
